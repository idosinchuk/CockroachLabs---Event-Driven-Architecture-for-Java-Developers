...
public class VehicleController {
    ...
    public void HandleRideEvent(KafkaMessage kafkaMessage) throws NotFoundException, DeserializationException {
        switch (kafkaMessage.getEventEnvelope().getEventType()) {
            case RideStarted.EVENT_TYPE:
                logger.info("[KAFKA] Received {}", RideStarted.EVENT_TYPE);
                RideStarted rideStarted = deserialize(mapper, kafkaMessage.getEventEnvelope().getData(), RideStarted.class);
            ...
            case RideEnded.EVENT_TYPE:
                ...
                RideEnded rideEnded = deserialize(mapper, kafkaMessage.getEventEnvelope().getData(), RideEnded.class);
                vehicleService.checkinVehicle(rideEnded.getVehicleId(), rideEnded.getLatitude(), rideEnded.getLongitude(), rideEnded.getBattery(), rideEnded.getEndTime());
                break;
            default:
                logger.warn("[KAFKA] Received an unknown message: {}", kafkaMessage.getEventEnvelope().getEventType());
        }
    }
}
